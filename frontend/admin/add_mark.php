...
//selecting Subjects from subject table
  ...
  $query="UPDATE `st_marks` SET `st_theory`= ?,`st_total`= ?,`isPassed`= ? WHERE `sub_id`= ? AND `s_id`= ? AND `sem`= ?";

    $stmt=$con->prepare($query);
  //bind parameters
  $stmt-> bind_param("iiiiii", $s_th,$fmark,$isPass,$sub_id,$s_id,$sem);

  //qry to insert in backlog table
                  ...
                  <div class="table-responsive table-bordered">
                    <table class="table table-hover text-center">
                      ...
                      <form action="" method="post">
                       ...
                       {
                         $qr1="SELECT * FROM `st_marks` WHERE `s_id` = '$s_id' AND `sub_id` = '$f[sub_id]'";
                         $es=mysqli_query($con,$qr1);
                          $f1=mysqli_fetch_assoc($es);
                      ...
                      <td class="align-middle text-center">
                        ...
                        <td class="align-middle">
                        <input type="number" class="cal form-control" value="<?=$f1['st_internal']?>"  id="int<?=$i;?>"  readonly> 
                        </td>
                        <td class="align-middle">
                        <input type="number" class="border-warning form-control" name="<?= $i.'fmark'?>" id="fmark<?=$i;?>" value="" readonly>
                        </td>
                        <td class="align-middle">
...
<script>
  ...
  for(let i=0; i< <?=$snum;?>; i++){
    ...
    var f = x+y;
    $('#fmark'+i).val(f);
    //if the total marks is less than the pass marks then the student is fail or pass
    if(x>0 && y>0){
    if(f < $('#pmark'+i).val())
    {
